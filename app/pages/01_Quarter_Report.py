...
st.set_page_config(page_title="Quarter Report", page_icon="📊", layout="wide")
st.title("📊 Quarter Report")
...
with col2:
    quarter_opt = st.radio("Quarter", ["Q1", "Q2", "Q3", "Q4", "Full Year"], horizontal=True)
with col3:
    use_custom = st.checkbox("Custom date range")
    if use_custom:
        c1, c2 = st.columns(2)
        custom_start = c1.date_input("From", datetime(year, 1, 1))
        custom_end = c2.date_input("To", datetime(year, 12, 31))
...
m1.metric("Total Income", f"€ {grand.get('total_income', 0):,.2f}")
m2.metric("Total Fees", f"€ {grand.get('total_fee', 0):,.2f}")
net = round(grand.get("total_income", 0) - grand.get("total_fee", 0), 2)
m3.metric("Net Income", f"€ {net:,.2f}")
...
for col_widget, region_key, label in [
    ...
    col_widget.metric(label, f"€ {income:,.2f}", f"Fees: €{fees:,.2f} | {count} tx")
...
for tab, geo_key in zip(tabs, geo_tab_keys):
    with tab:
        table_rows = build_monthly_table(payments, geo_key, year, quarter)  # type: ignore[arg-type]
        if table_rows:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Coaching": st.column_config.NumberColumn("Coaching €", format="%.2f"),
                    "Newsletter": st.column_config.NumberColumn("Newsletter €", format="%.2f"),
                    "Illustrations": st.column_config.NumberColumn("Illustrations €", format="%.2f"),
                    "Total Income": st.column_config.NumberColumn("Total Income €", format="%.2f"),
                    "Coaching Fee": st.column_config.NumberColumn("Coaching Fee €", format="%.2f"),
                    "Newsletter Fee": st.column_config.NumberColumn("Newsletter Fee €", format="%.2f"),
                    "Illustrations Fee": st.column_config.NumberColumn("Illust. Fee €", format="%.2f"),
                    "Total Fee": st.column_config.NumberColumn("Total Fee €", format="%.2f"),
                },
            )
            ...
            if not total_row.empty:
                t = total_row.iloc[0]
                st.markdown(
                    f"**Total:** Income €{t['Total Income']:,.2f} | "
...
with val_col:
    ...
    if val_report["activity_errors"] == 0 and val_report["unknown_activity"] == 0:
        ...
        st.warning(
            f"⚠️ {val_report['activity_errors']} activity errors | "
            f"{val_report['unknown_activity']} unclassified"
        )
        if st.button("Show validation details"):
            st.json(val_report)
